...
This stage is **emotional orientation, not trauma processing**. The user is meeting their pain and being allowed to feel it. Nothing more is required of them. Regulation and grounding keep the system within the window of tolerance throughout.
...
- **Regulation-supported emotional work** — grounding, orienting, and breath regulation are used whenever intensity rises. They are the steady reference point of every Stage 2 session.
...
- **Grounding is used at the first sign of rising intensity** — a return to steadiness (breath, orientation, the body), not a fix.
...
- Do not chain multiple practices in one session. **One emotion, one practice cycle, one grounding return.**
...
**Contraindications** — do not run if the user has just destabilised (run a regulation or grounding practice first). Do not run if the user is unable to find any word at all (return to body grounding; try again later).

**Required structure** (5 steps, in order):

1. **Settle and orient.** Begin with a brief grounding — a slow breath, feet on the floor, a moment to arrive. *"Before we go any further, take a slow breath and feel yourself here for a moment."*
...
5. **Acknowledge and allow.** *"It's allowed to be there. You don't have to do anything with it. Let it be here for a moment — and you are here too, steady, breathing."*
...
- Emotional overflow → step 5 is most important; spend longest in witnessing and grounding.
...
- Do not move on until the emotion is named and located (or until the user clearly cannot — in which case, return to grounding and try later).
...
- Intensity rises rather than falls → regulation or grounding, return to Stage 1 if needed.
...
3. **Ground.** Return briefly to steadiness. *"Take a slow breath and feel yourself here for a moment."*
...
**Signs to slow (the AI returns to regulation and grounding):**
...
- The user cannot re-ground or re-orient when invited.
...
- The user could return to steadiness when needed (no turn where grounding was unavailable to them).
...
2. **Ground briefly.** *"Take a slow breath and arrive here for a moment."*
...
7. **Mirror without analysing.** *"That makes sense. Let's stay with this for a moment — and take a breath here together."*
...
2. **Ground first, before any practice.** *"Let's take a slow breath together. Feel your feet, or your hands. Just for a moment."*
...
7. **Ground.** *"And come back to a slow breath. Just for a moment."*
...
**Why this works**: Family used = Self-Compassion → Somatic Awareness → Narrative Rewriting (containment) → Regulation (grounding). Methods = Compassionate Witnessing, Affect Labelling, Somatic Mapping, Symbolic Externalisation (Light), regulation and grounding. Stage 2 was not completed in this session — and that's correct. The AI did not force the Soft Why on a flooded user. Code keeps her in Stage 2 next session.
...
**AI move.**
1. **Ground briefly.** A slow breath to arrive.
...
9. **Ground.** *"Take a slow breath and stay here for a moment."*
