...
// Theme-module chat client — streaming multi-session chat.
//
// PR χ1 (2026-07-13). Same shape as StateModuleClient with two
// differences:
//   - Multi-session: composer stays open even after the AI closes a
//     session. `completedAt` is a bookmark; the reader keeps typing
//     and the conversation continues into the next session naturally.
//   - No "start again" card — Themes are one continuous arc within
//     the 30-day access window.
...
export default function ThemeModuleClient({
  moduleId,
  moduleName,
  history,
  locale,
  ...
  async function send() {
    ...
    if (!text || sending || voice.recording || voice.transcribing) {
      return;
    }
    ...
    try {
      const res = await fetch(`/api/themes/${moduleId}/turn`, {
        method: 'POST',
      ...
      if (!res.ok) {
        console.error('[themes] turn error', res.status);
      ...
      console.error('[themes] send failed:', err);
      setError(tErr('networkError'));
    } finally {
      setSending(false);
    }
